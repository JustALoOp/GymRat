...
import type { IWorkoutSession, PerformedExercise, PerformedSet } from '../types/workoutSession';

const PerformedExerciseCard: React.FC<{ performedExercise: PerformedExercise }> = ({ performedExercise }) => {
    const totalVolume = useMemo(() =>
        performedExercise.sets.reduce((acc: number, set: PerformedSet) => acc + (set.weight * set.reps), 0),
    ...
    return (
        <Card sx={{ mb: 2, bgcolor: 'background.default' }}>
            <CardContent>
                ...
                <List dense>
                    {performedExercise.sets.map((set: PerformedSet, index: number) => (
...
const WorkoutDetailsPage: React.FC = () => {
    ...
    const totalSessionVolume = useMemo(() =>
        session?.performedExercises.reduce((acc: number, pEx: PerformedExercise) =>
            acc + pEx.sets.reduce((setAcc: number, set: PerformedSet) => setAcc + set.weight * set.reps, 0), 0) || 0,
    ...
    return (
        <Paper sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
            ...
            {session.performedExercises.map(pExercise => (
                <PerformedExerciseCard key={pExercise.exercise._id} performedExercise={pExercise} />
            ))}
